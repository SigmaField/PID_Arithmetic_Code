...
def main():
    ...
    for ID in subject_IDs:
        ...
        left_MIs         = {frozenset(pair):mi(useful_raw_data[pair[0]],useful_raw_data[pair[1]]) for pair in combinations(left_channels,2)} 
        right_MIs        = {frozenset(pair):mi(useful_raw_data[pair[0]],useful_raw_data[pair[1]]) for pair in combinations(right_channels,2) if not ('z' in pair[0] and 'z' in pair[1])} 
